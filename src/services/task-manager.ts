...
export class TaskManager {
  ...
  async createTask(description: string, owner?: string, dependencies?: string[]): Promise<Task> {
    const task: Task = {
  ...
  async updateTask(id: string, updates: Partial<Pick<Task, "status" | "owner" | "description" | "dependencies">>): Promise<Task | null> {
  ...
  formatTaskList(): string {
    ...
    return this.tasks.map(t => {
      const statusIcon = t.status === "done" ? "✅" : t.status === "blocked" ? "❌" : t.status === "in-progress" ? "⏳" : "⬜";
      const ownerStr = t.owner ? ` (@${t.owner})` : "";
      const depsStr = t.dependencies && t.dependencies.length > 0 ? ` [Depends on: ${t.dependencies.join(", ")}]` : "";
